...
static bool ghost_is_vpn_dev(const char *name)
{
	if (!name)
		return false;
	if (!strncmp(name, "tun", 3) || !strncmp(name, "ppp", 3) ||
	    !strncmp(name, "wg", 2) || !strncmp(name, "p2p", 3) ||
	    !strncmp(name, "clash", 5) || !strncmp(name, "v2ray", 5))
		return true;
	return false;
}
...
static int dev_seq_show(struct seq_file *seq, void *v)
{
	if (v == SEQ_START_TOKEN) {
		...
		if (current_uid().val >= 10000 && ghost_is_vpn_dev(dev->name))
			return 0;
