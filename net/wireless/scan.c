...
static inline int ghost_apply_wifi_rssi_jitter(int signal, enum cfg80211_signal_type type)
{
	...
	/* Pseudo-random jitter derived from CPU clock */
	jitter = ((int)(now & 7)) - 3; /* range -3 to +4 */

	if (type == CFG80211_SIGNAL_TYPE_MBM) {
		/* +/- 300 mBm (+/- 3 dBm) */
		signal += (jitter * 100);
	} else {
		/* 0 - 100 unspec scale */
				...
				gfp_t gfp)
{
	...
	tmp.pub.signal = ghost_apply_wifi_rssi_jitter(data->signal, wiphy->signal_type);
