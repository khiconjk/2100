...
{
	struct timekeeper *tk = &tk_core.timekeeper;
	struct timespec64 adjusted_ts, ts_delta, xt;
	unsigned long flags;
	int ret = 0;
...
out_audit:
	/* V100 observes offset injections without changing their result. */
	ghost_uptime_audit_timekeeping_inject_offset(ret);
	return ret;
}
...
int do_adjtimex(struct __kernel_timex *txc)
{
	...
	ghost_uptime_audit_adjtimex(txc->modes);
